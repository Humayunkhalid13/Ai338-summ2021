...
#printing the percentage of people that survived
...
#drawing a bar plot of Survivedvs.Age
...
#dropping the Age include for presently
...
#outline Fare values into bunches of numerical values
...
model_svc
kf_svc=KFold(n_splits=7,shuffle=True)
cv_svc=cross_val_score(model_svc, predictors, target, cv=kf_svc)
np.mean(cv_svc)
model_svc.fit(x_train,y_train)
accuracy_svc=model_svc.score(x_val,y_val) 
accuracy_svc
ids = test['PassengerId']
predictions =svc.predict(test.drop('PassengerId', axis=1))

#set the yield as a dataframe and change over to csv file named submission.csv
output =pd.DataFrame({ 'PassengerId' : ids, 'Survived': predictions })
output.to_csv('D:\Titanic/submissionsvc1.csv', index=False)
